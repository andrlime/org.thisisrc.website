...
const StudentRegistration = () => {
  ...
  const descriptions = [
    <span>
      Visual Presentation of students’ project. Examples of display include
      research posters, prototypes, art work, etc. Display submissions are for
      students only.
    </span>,
    <span>
      Involves a group of people gathered to discuss a topic in front of an
      audience. Teachers and students can submit a request to a panel head or
      panelist to one of these 4 panels:
      <ol>
        <li>
          <b>Panel 1:</b> Ethics in Research
        </li>
        <li>
          <b>Panel 2:</b> Action Research
        </li>
        <li>
          <b>Panel 3:</b> Disciplinary Face-off
        </li>
        <li>
          <b>Panel 4:</b> Open Panel
        </li>
      </ol>
      Please indicate which panel you are interested in and whether you want to
      be panel head or a panelist. If you choose the open panel option, please
      include a title of for the panel and a brief description.
    </span>,
    <span>
      An activity where people work together on a specific subject or topic.
      Examples of workshop ideas are: Coding, Research skills, Genetic Editing
      Methods. Teachers and students can submit a workshop topic.
    </span>
  ]
      ...
      >
        <div className={styles.wrapper}>
          <>
            ...
            <p>Success!</p>
            <p>
              Now you have to pay. Please send ¥100 to the following QR code and
              include your name and school as a payment note. We will send you a
              confirmation email once we confirm your payment. If you do not see
              a QR code, please send us an email and we will send the picture to
              you. You can email us at{' '}
              <a href='mailto:isrc@this.edu.cn'>isrc@this.edu.cn</a>.
            </p>
            <img src='../payment.jpg' />
          </>
        </div>
          ...
          <div className={styles.form}>
            ...
            <span>
              ...
              <input
                style={{
                  border: `2px solid ${
                    colors[studentEmail ? isValidEmail.current + 1 : 0]
                  }`
                }}
                value={studentEmail}
                onChange={(event) => {
                  setStudentEmail(event.target.value)
                  const rx = /((\w|[-]|[.])+[@]\w+([.]\w+)+)/g // tests if it's an email
                  isValidEmail.current =
                    event.target.value.match(rx) == event.target.value
                }}
              />
            </span>
            <br />
            ...
            <span>
              ...
              <textarea
                onChange={(event) => {
                  ...
                  const rx_words = /(\s)/g
                  const wc = [...event.target.value.matchAll(rx_words)]
              ...
              <span
                id={styles.wcword}
                style={{
                  color: projectDescriptionLength > 250 ? colors[2] : colors[0]
                }}
              >
            ...
            <span>
              {showDisTwo ? 'First ' : ''}Discipline:{' '}
              <select
                onChange={(event) =>
                  setProjectDisciplineOne(event.target.value)}
                style={{ border: `2px solid ${colors[0]}` }}
                value={projectDisciplineOne}
              >
                <option value='n'>Please Select</option>
              ...
              <button
                ...
                onClick={() => setShowDisTwo(!showDisTwo)}
              >
                {showDisTwo ? '–' : '+'}
              </button>
              <br />
            ...
            {showDisTwo
              ? (
                <span>
                  ...
                  <select
                    onChange={(event) =>
                      setProjectDisciplineTwo(event.target.value)}
                    style={{ border: `2px solid ${colors[0]}` }}
                    value={projectDisciplineTwo}
                  >
                    <option value='n'>Please Select</option>
                    <option value='Science'>Science</option>
                    <option value='Social Studies'>Social Studies</option>
            ...
            <button
              onClick={() => {
                ...
                if (
                  ...
                  projectType &&
                  projectDescription &&
                  projectDisciplineOne &&
                  isValidEmail.current &&
                  projectDescriptionLength < 250 &&
                  showDisTwo == projectDisciplineTwo
                ) {
                  const student = {
                  ...
                  axios
                    ...
                    .catch((err) => {
                      setShowModal(false)
                      setMessage(
                        'There was an error. Please try again. If this continues, please send us an email with your registration info.'
                      )
                  ...
                  setMessage(
                    "You didn't complete the form or your email was invalid."
                  )
                }
              }}
            >
